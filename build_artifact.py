...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.1-lua-term-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.1-lua-term-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.1-lua-term-*.xml".')
...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-lua-term-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-lua-term-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.4-lua-term-*.xml".')
